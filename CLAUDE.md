...
- **All components are stateless arrow functions returning JSX.** No props, no hooks, no
  client-side state anywhere. Jobs and education entries live as data in `data/cv.js`,
  imported by `WorkHistory.js` and `Education.js`; other content (links, about text) is
  hardcoded literal JSX inside the component — updating those means editing the component,
  not a data file.
